...
function activate(context) {
    ...
    async function compareWithLeft(a) {
        let rightPath = "";
        let blnRRO = false;
        let rightLabel = "";
        if (a) {
        ...
        if (!a) //If not run by right clicking on an editor tab (when run from command palette or keybinding)
         {
            if (!vscode.window.activeTextEditor) {
            ...
            else //but there is a text editor active
             {
                ...
                if (vscode.window.activeTextEditor.document.isUntitled) { //No saved version version
                    ...
                    if (!vscode.window.activeTextEditor.document.isDirty) //If it hasn't changed, ask for confirmation
                     {
                        ...
                        vscode.window.showWarningMessage("\"" + splitPath[splitPath.length - 1] +
                            ...
                            .then((answer, docPath = verifyIsString(fileName), document = doc) => {
            ...
            while (vscode.window.tabGroups.activeTabGroup.activeTab !== startingTab || firstLoop) //Loop through all editors to return to the starting one
             {
                ...
                if (!firstLoop || aEditor === undefined) {
    ...
    registerCommand('.selectLeftText', async () => {
        ...
        writeLeftPath(fs_1.default.realpathSync(tempPath), true, "Left Text");
        ...
        switch (a.type) {
            ...
            case 0:
                ...
                if (compareTarget === undefined) {
                    ...
                    if (fs_1.default.existsSync(a.resourceUri.fsPath) && case0Staged) {
    ...
    registerCommand('.compareTextToClipboard', async () => {
        ...
        let tempPath = uri.fsPath;
        temporaryFiles.push(tempPath);
        let comparePath = fs_1.default.realpathSync(tempPath);
        if (comparePath !== "") {
            let clipboardPath = await saveClipboardToFile();
            if (clipboardPath === false) {
                vscode.window.showErrorMessage("Error: Clipboard does not contain readable text");
    ...
    async function compareSelected(a) {
        const items = a[1];
        if (items.length === 2) {
    ...
    async function gitCompareHelper(strPath, command) {
        ...
        let filePath = false;
        await (0, simple_git_1.simpleGit)(directory).outputHandler((_command, standardOut) => {
    ...
    async function openBC(options = "", names, ...files) {
    ...
    function isPro() {
        if (strOS === "win32") {
            ...
            for (var version in versionNumbers) {
                const bcRegistryFolder = "SOFTWARE\\Scooter Software\\Beyond Compare";
                try {
                    let strThreeWayCompareAllowed = vsWinReg.GetStringRegKey('HKEY_CURRENT_USER', bcRegistryFolder + versionNumbers[version], 'SupportsMerge');
    ...
    function writeLeftPath(strPath, blnTempFile = false, strNewLeftName = "") {
    ...
    function readLeftPath() {
        //Read path
        let readFilePath = verifyIsString(globalState.get("leftPath"));
        let leftPreserved = globalState.get("leftIsPreserved");
        let leftName = verifyIsString(globalState.get("leftName"));
        ...
        if (readFilePath === '' || !fs_1.default.existsSync(readFilePath)) //No left item or left item was deleted/is inaccessible
         {
            vscode.commands.executeCommand('setContext', extensionName + '.leftSelected', false);
            vscode.commands.executeCommand('setContext', extensionName + '.leftFolderSelected', false);
            readFilePath = '';
        }
        else if (fs_1.default.statSync(readFilePath).isFile()) //Left item is file
         {
            vscode.commands.executeCommand('setContext', extensionName + '.leftSelected', true);
            vscode.commands.executeCommand('setContext', extensionName + '.leftFolderSelected', false);
        }
    ...
    async function openFromDiffHelper(tabInput) {
        let rightFilePath = "";
        let leftFilePath = "";
        let options = "";
        let leftLabel = "";
    ...
    function getLabelForOpenFromDiff(uri) {
    ...
    function openFromDiffHelper2(uri, filePath) {
    ...
    function extractExtAndMakeRandomFilePath(originalPath) {
        let extension;
        try {
    ...
    async function isUriSameAsDisk(uri1) {
        let uri2;
        try {
            uri2 = vscode.Uri.parse(uri1.path);
            fs_1.default.accessSync(uri2.fsPath, fs_1.default.constants.R_OK); //Throws if file can't be read
        }
        catch {
            return false;
        }
        ...
        if (fs_1.default.statSync(uri2.fsPath).size !== (await vscode.workspace.fs.stat(uri1)).size) {
            return false;
        }
    ...
    async function copyRemoteFileAsTemp(a) {
        try {
    ...
    function deleteFileIfTemp(path) {
        let fileIsTemp = false;
        let intFoundIndex = -1; //'i' will always be set to something else in the cases where it's needed
        ...
        if (fileIsTemp && fs_1.default.existsSync(path)) {
            fs_1.default.promises.unlink(path);
...
function verifyIsString(strInput, strDefault = "") {
...
function deactivate() {
    ...
    temporaryFiles.forEach((file) => //Delete all temporary files created by this extension
     {
        if (fs_1.default.existsSync(file) && file !== leftPath) //but not the left file
         {
            fs_1.default.promises.unlink(file); //In theory, this will never happen - all temp files other than the current left should have already been deleted
        }
    });
}
